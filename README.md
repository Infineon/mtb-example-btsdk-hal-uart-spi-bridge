...
## Overview
This application implements the SPI Master and act as a UART-SPI Bridge. The uart\_spi\_bridge sample application demonstrates the signaling protocol to be implemented on the customer MCU to communicate over SPI using WICED\_HCI messages. The bridge application implements the following functionalities:

1. Receives WICED\_HCI Commands from MCU/Host over PUART and forwards the commands over SPI to the Slave device.
2. Receives WICED\_HCI Events from the connected WICED SPI device and forwards them over PUART to the connected host.
3. Intercepts the HCI\_CONTROL\_AUDIO\_DATA request from the WICED SPI device and responds with a stored sine wave to the WICED device.

## Instructions
To demonstrate the app, work through the following steps:

1. Plug the WICED Bluetooth evaluation board into your computer
2. Build and download the uart\_spi\_bridge application to a WICED board (SPI Master).
3. Build and download the hci\_av\_source application to another WICED board (SPI slave).
4. Connect the SPI signals between master and slave as shown below.
5. Open the Client Control application. Select the PUART port on the board running the uart\_spi\_bridge application at 115200 baud rate without flow control
6. Reset the board that runs the uart\_spi\_bridge application.
7. Reset the board that runs the watch application.
8. Use the ClientControl application to send various commands.

Pins used on CYW920706WCDEVAL (for example):
