...
## Overview
This application implements the SPI Master and act as a UART-SPI Bridge. The uart\_spi\_bridge sample application demonstrates the signaling protocol to be implemented on the customer MCU to communicate over SPI using AIROC&#8482; HCI messages. The bridge application implements the following functionalities:

1. Receives AIROC&#8482; HCI Commands from MCU/Host over PUART and forwards the commands over SPI to the Slave device.
2. Receives AIROC&#8482; HCI Events from the connected AIROC&#8482; SPI device and forwards them over PUART to the connected host.
3. Intercepts the HCI\_CONTROL\_AUDIO\_DATA request from the AIROC&#8482; SPI device and responds with a stored sine wave to the AIROC&#8482; device.

## Instructions
To demonstrate the app, work through the following steps:

1. Plug the AIROC&#8482; Bluetooth&#174; evaluation board into your computer
2. Build and download the uart\_spi\_bridge application to a AIROC&#8482; board (SPI Master).
3. Build and download the hci\_av\_source application to another AIROC&#8482; board (SPI slave).
